...
public class PrescriptionService {
    ...
    // 1) Save prescription (prevents duplicates for the same appointment)
    public ResponseEntity<Map<String, String>> savePrescription(Prescription prescription) {
        Map<String, String> response = new HashMap<>();
        try {
            if (prescription == null || prescription.getAppointmentId() == null) {
                response.put("message", "Invalid prescription or missing appointmentId");
            ...
            logger.error("Failed to save prescription", ex);
    ...
    public ResponseEntity<Map<String, Object>> getPrescription(Long appointmentId) {
        ...
        try {
            if (appointmentId == null) {
                response.put("message", "appointmentId is required");
                return ResponseEntity.badRequest().body(response);
            }
            ...
            // If multiple records exist, return the first one (business rule can be adjusted if needed)
            response.put("prescription", prescriptions.get(0));
            return ResponseEntity.ok(response);
        } catch (Exception ex) {
            logger.error("Failed to retrieve prescription for appointmentId={}", appointmentId, ex);
